...
| WHAT was the outcome? | Application report-back (not implemented) |
...
`evaluateAuthorized()` in `apps/api/src/services/application-preflight.ts`:

- HMAC + timestamp + nonce
- `denyImpersonation`
- kill categories
- `delete` / `destroy` / `drop` → DENY
- HIGH / TOOL → REQUIRE_APPROVAL
- GOVERNED_DECISION / INFORMATIONAL fail-open when secret unset
- HIGH_RISK / TOOL_ACTION fail-closed when secret unset
...
| Cost/outcome attribution | Who spent, what resulted | Audit has applicationId/agentId/actorId/operation | No executionId↔preflightId, no outcome | CORE SUPPORTING | Wave 4–5 after Wave 1 commit |
...
| G16 | Outcome | No `executionId` / `resultStatus` / `outcomeStatus` / `actualCost` report-back. | — | MISSING | Allowed ≠ completed | G1, G13 | Wave 4 contract |
...
| G18 | Unknown / shadow Agents | Null agentId is honest unknown. No observed-vs-known set. | applicationOwnedAgentId | MISSING | Shadow activity invisible | G1, G22 | CTRL-018 observe only |
...
| G24 | Failure / recovery | Fail-open vs fail-closed by operation class. Duplicate/retry via nonce/idempotency. Control-down: app skip if secret unset. | evaluateAuthorized comments + tests | PARTIAL | Silent skip of governed hops | Document per class | Do not globally fail-closed |
...
| G27 | Incident / investigation | Reconstructable when audit has applicationId+agentId+actorId+operation+decision. Missing execution/outcome. | audit input | PARTIAL | Incomplete incident story | G13, G16 | Wave 4+6 |
...
| --------- | -------- | ------ |
| Atlas API secret unset, GOVERNED_DECISION / INFORMATIONAL | FAIL OPEN (skip preflight client-side) | Local/dev siblings must not hard-stop |
| Secret unset, HIGH_RISK / TOOL_ACTION | FAIL CLOSED | Destructive / tool hops must not run ungoverned |
...
| Atlas API down | Client skip only if fail-open class; else block | Matches evaluateAuthorized |
| Approval unavailable, HIGH/TOOL | REQUIRE_APPROVAL / no ALLOW | Fail closed for those classes |
...
PARTIAL:                      15
MISSING:                      10  (G4, G5, G8, G9, G15, G16, G18, G19, G21, G25)
...
11. **What actually happened:** PARTIAL — authorized is recorded; executed is not (siblings).
