...
const localWebServers = [
  {
    command: "node e2e/stage9/reset-local-identities.mjs && pnpm --filter @atlas/api dev",
    url: "http://127.0.0.1:4000/api/v1/health",
    reuseExistingServer: false,
    timeout: 180_000,
    env: envWith({
      ATLAS_OPERATOR_EMAILS: STAGE9_OPERATOR_EMAILS_ENV,
      // Fresh local auth file so bootstrapRole runs while the operator email
      // is set. The reset script removes the same emails from local Supabase
      // first; otherwise /auth/me returns the old Supabase id. Do not set
      // SUPABASE_SERVICE_ROLE_KEY=replace-me here: Apply needs the live
      // approval store, and that sentinel makes it return 503.
