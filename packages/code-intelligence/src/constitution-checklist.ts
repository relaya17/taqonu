...
export const CONSTITUTION_CHECKLIST: readonly ConstitutionChecklistItem[] = [
  ...
  // Full 23-domain coverage MVP — deepen thin domains from ADR inventory
  item({
    id: "nav.breadcrumbs",
    domain: "NAVIGATION",
    title: "Breadcrumbs / secondary wayfinding",
    description: "Breadcrumb or equivalent secondary navigation for deep routes",
    severityIfMissing: "LOW",
    profiles: [...S],
    detectorKey: "nav_breadcrumbs",
    remediationHint: "Add breadcrumbs for nested product routes when hierarchy is deep",
  }),
  item({
    id: "footer.contact_copyright",
    domain: "FOOTER",
    title: "Footer contact / copyright when public",
    description: "Contact mailto or copyright notice for public-facing footers",
    severityIfMissing: "LOW",
    profiles: ["SAAS", "MARKETING_SITE", "PAYMENTS"],
    detectorKey: "footer_contact_copyright",
    remediationHint: "Add contact + © only when the product is public-facing",
  }),
  item({
    id: "resp.viewport_overflow",
    domain: "RESPONSIVE",
    title: "Viewport / overflow guards",
    description: "viewport meta, overflow-x, max-width, or fluid layout clamps",
    severityIfMissing: "MEDIUM",
    profiles: [...W, "SAAS"],
    detectorKey: "resp_viewport_overflow",
    remediationHint: "Prevent horizontal overflow; set viewport; clamp fluid widths",
  }),
  item({
    id: "ui.shared_primitives",
    domain: "UI_CONSISTENCY",
    title: "Shared UI primitives",
    description: "Shared Button/Input/components package — not one-off controls per page",
    severityIfMissing: "MEDIUM",
    profiles: [...S],
    detectorKey: "ui_shared_primitives",
    remediationHint: "Centralize controls in components/ui or a design-system package",
  }),
  item({
    id: "ux.error_confirm",
    domain: "UX",
    title: "Error feedback + destructive confirms",
    description: "Error toasts/alerts and confirm dialogs for destructive actions",
    severityIfMissing: "MEDIUM",
    profiles: [...S],
    detectorKey: "ux_error_confirm",
    remediationHint: "Show errors clearly; confirm deletes and irreversible actions",
  }),
  item({
    id: "perf.caching",
    domain: "PERFORMANCE",
    title: "Caching / CDN / revalidate",
    description: "Cache-Control, CDN, or framework revalidate/cache signals",
    severityIfMissing: "LOW",
    profiles: ["WEB_APP", "SAAS", "MARKETING_SITE", "API_SERVICE"],
    detectorKey: "perf_caching",
    remediationHint: "Cache static/CDN assets; set revalidate for semi-static pages",
  }),
  item({
    id: "db.indexes_backup",
    domain: "DATABASE",
    title: "Indexes and backup/restore story",
    description: "Index definitions plus backup/restore or retention notes",
    severityIfMissing: "HIGH",
    profiles: ["SAAS", "API_SERVICE", "PAYMENTS"],
    detectorKey: "db_indexes_backup",
    remediationHint: "Index hot queries; document backup/restore and retention",
  }),
  item({
    id: "ext.api_failure_modes",
    domain: "EXTERNAL_APIS",
    title: "External API failure / version handling",
    description: "Timeouts, retries, rate-limit handling, or API version pins for vendors",
    severityIfMissing: "HIGH",
    profiles: ["SAAS", "PAYMENTS", "AI_PRODUCT", "API_SERVICE"],
    detectorKey: "ext_api_failure_modes",
    remediationHint: "Handle vendor failures: timeout, retry, 429, deprecation/version",
  }),
  item({
    id: "deps.license_audit",
    domain: "DEPENDENCIES",
    title: "License / dependency audit signal",
    description: "LICENSE file, Dependabot/Renovate, or npm/pnpm audit wiring",
    severityIfMissing: "MEDIUM",
    profiles: [...A],
    detectorKey: "deps_license_audit",
    remediationHint: "Commit LICENSE; enable Dependabot/audit — not “newest = best”",
  }),
  item({
    id: "hyg.console_todo",
    domain: "CODE_HYGIENE",
    title: "Console / TODO triage (no fake-clean)",
    description: "Flag console.* debug logs; TODOs OK if tracked — don’t purge for vanity",
    severityIfMissing: "LOW",
    profiles: [...A],
    detectorKey: "hyg_console_todo",
    remediationHint: "Prefer structured logs; triage TODOs — never fake-clean checklists",
  }),
  item({
    id: "i18n.locale_routes",
    domain: "I18N",
    title: "Locale routes wired (not dropdown-only)",
    description: "[locale] routes or message catalogs — not only a language select",
    severityIfMissing: "HIGH",
    profiles: ["SAAS", "WEB_APP", "MARKETING_SITE"],
    detectorKey: "i18n_locale_routes",
    remediationHint: "Wire real locale routes + catalogs for he/en/ar",
  }),
  item({
    id: "legal.retention_deletion",
    domain: "LEGAL_PRIVACY",
    title: "Retention / deletion / export",
    description: "Data retention, erasure, or export signals for public products",
    severityIfMissing: "HIGH",
    profiles: ["SAAS", "MARKETING_SITE", "PAYMENTS"],
    detectorKey: "legal_retention_deletion",
    remediationHint: "Document retention; support deletion/export where required",
  }),
  item({
    id: "sec.scanner_sarif",
    domain: "SECURITY",
    title: "Scanner / SARIF in CI or security feed",
    description: "Semgrep, CodeQL, Trivy, or SARIF ingest wired into the pipeline",
    severityIfMissing: "HIGH",
    profiles: ["SAAS", "WEB_APP", "API_SERVICE", "PAYMENTS"],
    detectorKey: "sec_scanner_sarif",
    remediationHint: "Add Semgrep/CodeQL and POST /api/v1/security/sarif → SECURITY evidence",
  }),
  item({
    id: "obs.metrics_export",
    domain: "OBSERVABILITY",
    title: "Metrics export (/metrics or Prometheus)",
    description: "Process metrics available for scrape or durable NDJSON",
    severityIfMissing: "MEDIUM",
    profiles: ["SAAS", "API_SERVICE", "WEB_APP"],
    detectorKey: "obs_metrics_export",
    remediationHint: "Expose Prometheus text and/or durable metrics.ndjson",
  }),
  item({
    id: "deploy.provider_feed",
    domain: "DEPLOYMENT",
    title: "Deploy provider → evidence feed",
    description: "Vercel/Render (or similar) observe path into DEPLOYMENT evidence",
    severityIfMissing: "MEDIUM",
    profiles: ["SAAS", "WEB_APP", "API_SERVICE"],
    detectorKey: "deploy_provider_feed",
    remediationHint: "Wire POST /api/v1/providers/vercel|render/observe after deploys",
  }),
];
