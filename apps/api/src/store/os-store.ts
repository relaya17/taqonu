...
class OsStore {
  ...
  /**
   * `ownerId` (P0 tenant-isolation fix): when provided, only memories owned
   * by that caller are returned. Omit only for trusted internal callers that
   * already scope by project ownership elsewhere (e.g. agent-context
   * assembly keyed by a project the caller was already verified to own).
   * Any caller reachable from an HTTP route MUST pass `ownerId` — this is
   * the actual enforcement point, not a decorative filter.
   */
  getMemories(projectId: string, ownerId?: string): Memory[] {
    this.ensureLoaded();
    const list = this.memories.get(projectId) ?? [];
    return ownerId === undefined
      ? list
      : list.filter((memory) => memory.ownerId === ownerId);
  }
