...
describe("D3 Web/Studio navigation and Checks consolidation", () => {
  it("keeps every existing Web nav route in AppShell PATHS", () => {
    ...
    expect(dashboard).toContain('studioCheckHref("qa", projectId)');
    expect(dashboard).toContain('studioCheckHref("processAudit", projectId)');
    expect(dashboard).not.toContain('href="/studio"');
    expect(dashboard).not.toContain('href="/readiness"');
    expect(dashboard).not.toContain('href="/health"');
    ...
    expect(projectsPage).toContain("disabled={!project.workspaceRoot}");
  ...
  it("surfaces PSA memory in Studio without merging CODE_ENGINEER ask-agent", () => {
    ...
    expect(panel).toContain("/api/v1/supervising-agent/explain");
    expect(panel).toContain("/api/v1/supervising-agent/recommend");
    expect(panel).toContain("/api/v1/supervising-agent/escalate");
    expect(panel).toContain("/api/v1/supervising-agent/request");
    expect(panel).toContain("buildPsaGovernedRequest");
    expect(panel).toContain("FABRIC_AGENT_IDS");
    expect(panel).not.toContain("/api/v1/studio/ask-agent");
    expect(panel).not.toContain("/api/v1/studio/file/move");
    expect(panel).not.toContain("/api/v1/code/patches");
    expect(panel).not.toContain("decide-and-execute");
    ...
    expect(studio).toContain("/api/v1/studio/file/move");
...
describe("D2 Studio approve/apply does not skip human approval", () => {
  it("apply is gated on APPROVED status in the Studio workflow helper", () => {
    ...
    expect(workflow).toContain("patchDecideAndExecutePath");
    expect(workflow).toContain('action: "apply"');
    expect(workflow).toContain('action: "rollback"');
    expect(workflow).not.toContain("/api/v1/approvals/");
    ...
    expect(patchesPanel).toContain("deskPatchVerifyPath");
    expect(patchesPanel).toContain("formatPatchVerifyLabel");
    expect(patchesPanel).not.toContain("/api/v1/remediation/drafts/");
