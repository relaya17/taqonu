...
const tmpDir = mkdtempSync(join(tmpdir(), "atlas-deploy-feeds-route-test-"));
...
const { registerDeployFeedRoutes } = await import("./deploy-feeds.js");
...
beforeAll(async () => {
  app = await buildRouteTestApp(registerDeployFeedRoutes);
});
...
describe("POST /api/v1/feeds/vercel", () => {
  it("401s when not signed in", async () => {
    const projectId = makeProject(null);
    getRequestUser.mockReturnValue(null);
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/feeds/vercel",
      payload: {
        projectId,
        projectName: "my-app",
  ...
  it("201s for the owner (no regression)", async () => {
    ...
    expect(res.json().feed.status).toBeDefined();
  });
});

describe("POST /api/v1/feeds/render", () => {
  it("401s when not signed in", async () => {
    const projectId = makeProject(null);
    getRequestUser.mockReturnValue(null);
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/feeds/render",
      payload: {
        projectId,
...
describe("GET /api/v1/feeds/:projectId/deployment", () => {
