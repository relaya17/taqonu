...
const tmpDir = mkdtempSync(join(tmpdir(), "atlas-observer-route-test-"));
...
const { registerObserverRoutes } = await import("./observer.js");
...
beforeAll(async () => {
  app = await buildRouteTestApp(registerObserverRoutes);
});
...
describe("GET /api/v1/projects/:id/observer", () => {
...
describe("GET /api/v1/observer/state", () => {
  it("200s without auth when no projectId is given (no tenant data involved, unchanged this round)", async () => {
    const res = await app.inject({ method: "GET", url: "/api/v1/observer/state" });
    expect(res.statusCode).toBe(200);
  });

  it("401s when a projectId is given and caller is not signed in (security fix)", async () => {
    const projectId = makeProject(null);
    getRequestUser.mockReturnValue(null);
    const res = await app.inject({
      method: "GET",
...
describe("POST /api/v1/observe/cycle", () => {
  it("401s when not signed in", async () => {
    getRequestUser.mockReturnValue(null);
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/observe/cycle",
      payload: {},
    });
    expect(res.statusCode).toBe(401);
  });
...
describe("POST /api/v1/observer/bugs", () => {
  it("401s when not signed in", async () => {
    getRequestUser.mockReturnValue(null);
    const res = await app.inject({
      method: "POST",
