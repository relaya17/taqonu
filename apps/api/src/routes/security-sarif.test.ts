...
const tmpDir = mkdtempSync(join(tmpdir(), "atlas-security-sarif-route-test-"));
...
const { registerSecuritySarifRoutes } = await import("./security-sarif.js");
...
beforeAll(async () => {
  app = await buildRouteTestApp(registerSecuritySarifRoutes);
});
...
describe("POST /api/v1/security/sarif", () => {
  it("401s when not signed in (security fix — this route previously had ZERO auth)", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/security/sarif",
      payload: { projectId, sarif: sampleSarif },
    });
  ...
  it("201s for the owner and records SECURITY evidence (no regression)", async () => {
    const owner = signedInUser();
    const projectId = makeProject(owner);
    getRequestUser.mockReturnValue(owner);
    const res = await app.inject({
    ...
    expect(body.evidenceIds).toHaveLength(1);
  });
});
