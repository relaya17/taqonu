...
/**
 * ENTITY-LEVEL gate for the 5 mutating routes below (connect/disconnect ×
 * github+local, plus github import and local scan), independent of the
 * `requireSignedInForWrite` caller-identity gate already applied by each
 * one. Source connections are "integrations" in `BusinessEntityType`'s own
 * doc comment, so every mutation here is CONFIGURATION — mirrors the exact
 * pattern used by `plugins.ts`'s `enforcePluginEntityAuthz`: a signed-in
 * caller's own REST write is treated as sufficient authorization
 * (`writeGateOpen`/`approved` hardcoded `true`, no separate approval round
 * trip manufactured here), and both `DENIED`/`APPROVAL_REQUIRED` collapse
 * to the same 403 `FORBIDDEN`.
 */
function enforceConnectionEntityAuthz(
...
export async function registerConnectionRoutes(
  ...
  app.post("/api/v1/connections/github", async (request, reply) => {
    ...
    enforceConnectionEntityAuthz("CREATE", "connections.github.connect");
  ...
  app.delete("/api/v1/connections/github", async (request) => {
    await requireSignedInForWrite(app, request);
    enforceConnectionEntityAuthz("DELETE", "connections.github.disconnect");
  ...
  app.post("/api/v1/connections/github/import", async (request, reply) => {
    await requireSignedInForWrite(app, request);
    enforceConnectionEntityAuthz("EXECUTE", "connections.github.import");
  ...
  app.post("/api/v1/connections/local", async (request, reply) => {
    await requireSignedInForWrite(app, request);
    enforceConnectionEntityAuthz("CREATE", "connections.local.connect");
  ...
  app.delete("/api/v1/connections/local", async (request) => {
    await requireSignedInForWrite(app, request);
    enforceConnectionEntityAuthz("DELETE", "connections.local.disconnect");
  ...
  app.post("/api/v1/connections/local/scan", async (request, reply) => {
    await requireSignedInForWrite(app, request);
    enforceConnectionEntityAuthz("EXECUTE", "connections.local.scan");
