 ...
 */
export interface CostIntelligenceDailyPoint {
  /** UTC calendar date, `YYYY-MM-DD`, derived from each entry's `at` timestamp. */
  date: string;
  totalUsd: number;
  dispatchCount: number;
}

export interface CostIntelligenceProjectDailySeries {
  projectId: string | null;
  points: CostIntelligenceDailyPoint[];
}

function toUtcDateKey(atValue: unknown): string | null {
  if (typeof atValue !== "string" || atValue.length === 0) return null;
...
/**
 * Same `"agents.dispatch"` audit entries `computeCostIntelligenceSummary`
 * reads, regrouped into a per-project daily cost time series (summing
 * `toEntryCostUsd` per UTC calendar day) instead of a single all-time
 * total. This is the real, already-persisted data shape a statistical
 * anomaly detector (`@atlas/agent-core`'s `detectAnomalies`) needs — one
 * numeric value per time bucket — rather than a single scalar per
 * project. Entries with an unparseable/missing `at` are skipped (there is
 * no day to bucket them into); this mirrors the rest of this file's
 * "never invent data" stance.
 *
 * Sorted ascending by date within each project, which is the order
 * `detectAnomalies` requires (it computes a *trailing* baseline window,
 * so out-of-order input would silently corrupt the baseline).
 */
export function computeCostIntelligenceDailySeriesByProject(input?: {
  projectId?: string | null;
}): CostIntelligenceProjectDailySeries[] {
  ...
  const buckets = new Map<
    string | null,
    Map<string, { totalUsd: number; dispatchCount: number }>
  >();
  ...
  for (const raw of entries) {
    ...
    const dateKey = toUtcDateKey(record.at);
    if (dateKey === null) continue;

    const cost = toEntryCostUsd(record);
    const projectBucket = buckets.get(projectId) ?? new Map();
    const dayBucket = projectBucket.get(dateKey) ?? {
      totalUsd: 0,
      dispatchCount: 0,
    };
