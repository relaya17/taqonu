...
export function syncProcessAuditToMemory(doc: ProcessAuditDocument): Memory {
  ...
  // Secret-redaction gate (mirrors routes/memory.ts, routes/agent.ts): the
  // process-audit sections/verdictReason are free-text scan output and can
  // plausibly echo a leaked secret found in logs/env dumps/stack traces —
  // never persist a raw credential/token into Memory.
  const safeStatement = redactSecrets(statement);
