...
/**
 * ENTITY-LEVEL gate for the 2 mutating routes below. Both entity types get
 * the same self-approved signed-in-human-write pattern as `memory.ts`/
 * `evidence.ts`: `writeGateOpen`/`approved` hardcoded `true`, both
 * `DENIED`/`APPROVAL_REQUIRED` collapse to 403 `FORBIDDEN`.
 */
function enforceArtifactEntityAuthz(
  entityType: "DOCUMENT" | "RECORD",
...
export async function registerArtifactRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/artifacts", async (request, reply) => {
    ...
    enforceArtifactEntityAuthz("DOCUMENT", "CREATE", "artifacts.upload");
  ...
  app.post("/api/v1/assists/runs", async (request, reply) => {
    ...
    // gate.
    await requireSignedInForWrite(app, request);
    enforceArtifactEntityAuthz("RECORD", "EXECUTE", "assists.run");
