 ...
 * `requireUser` identity gate already applied by each one. A decision
 * record is the `RECORD` bucket entity per `BusinessEntityType`'s own doc
 * comment. Same self-approved signed-in-human-write pattern as
 * `projects.ts`/`memory.ts`: `writeGateOpen`/`approved` hardcoded `true`,
 * both `DENIED`/`APPROVAL_REQUIRED` collapse to 403 `FORBIDDEN`.
 */
function enforceDecisionEntityAuthz(
...
export async function registerDecisionRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/decisions", async (request, reply) => {
    await requireUser(app, request);
    enforceDecisionEntityAuthz("CREATE", "decisions.create");
  ...
  app.post("/api/v1/decisions/:id/transition", async (request, reply) => {
    await requireUser(app, request);
    enforceDecisionEntityAuthz("UPDATE", "decisions.transition");
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params);
