...
// Same stubbing mechanism as `apps/api/src/routes/gates.test.ts` /
// `apps/api/src/routes/byo-cloud.test.ts`: spread the real `@atlas/agent-core`
// module (so `registerPlugin`/`approvePlugin`/etc. and the real
// `authorizeEntityAction` both still work unmodified by default) and only
// stub `authorizeEntityAction` so individual tests can force a DENIED /
// APPROVAL_REQUIRED decision without needing to fake `mode`/`writeGateOpen`
// plumbing end-to-end.
 ...
 * Previously these routes were gated ONLY by `requireAdmin` (role) with
 * ZERO entity-policy (`authorizeEntityAction`) call, despite a plugin
 * representing a third-party capability grant. Each `it` below proves,
 * per route: (1) a DENIED decision from the Policy Engine genuinely
 * blocks the request with 403, even though `requireAdmin` alone would
 * have let it through, and (2) `authorizeEntityAction` is actually
 * invoked with the entityType/action mapping this route claims to use.
 * The pre-existing tests above (which keep the default
 * `authorizeEntityAction.mockReturnValue(undefined)` — i.e. fall through
 * to the real implementation) already prove the normal-admin-ALLOWED path
 * still works end to end (e.g. the "full lifecycle" test exercises
 * register -> approve -> enable -> disable -> uninstall all succeeding
 * with the real Policy Engine wired in).
 */
describe("Policy Engine (authorizeEntityAction) gate on mutating routes", () => {
  it("403s POST /api/v1/plugins (register) when the Policy Engine denies CONFIGURATION.CREATE", async () => {
    ...
    expect(authorizeEntityAction).toHaveBeenCalledWith(
      "CONFIGURATION",
      "CREATE",
  ...
  it("403s register — and does not register the plugin — when the Policy Engine returns APPROVAL_REQUIRED (not treated as allowed)", async () => {
    authorizeEntityAction.mockReturnValue({
    ...
    const get = await app.inject({
      method: "GET",
      url: "/api/v1/plugins/sample-plugin",
    });
    expect(get.statusCode).toBe(404);
  });

  it("403s POST .../approve when the Policy Engine denies CONFIGURATION.UPDATE", async () => {
    await registerViaApi();
    authorizeEntityAction.mockReturnValue({
      decision: "DENIED",
      reason: "test-forced denial",
  ...
  it("403s POST .../reject when the Policy Engine denies CONFIGURATION.UPDATE", async () => {
    await registerViaApi();
    authorizeEntityAction.mockReturnValue({
      decision: "DENIED",
      reason: "test-forced denial",
  ...
  it("403s POST .../enable when the Policy Engine denies CONFIGURATION.UPDATE, even for an already-APPROVED plugin", async () => {
    await registerViaApi();
    await app.inject({
      method: "POST",
      url: "/api/v1/plugins/sample-plugin/approve",
      payload: { reason: "Looks safe." },
    });
    authorizeEntityAction.mockReturnValue({
      decision: "DENIED",
      reason: "test-forced denial",
    ...
    // The plugin is untouched — still APPROVED, not ENABLED.
    authorizeEntityAction.mockReturnValue(undefined);
    const get = await app.inject({
      method: "GET",
      url: "/api/v1/plugins/sample-plugin",
  ...
  it("403s POST .../disable when the Policy Engine denies CONFIGURATION.UPDATE, even for an already-ENABLED plugin", async () => {
    await registerViaApi();
    await app.inject({
      method: "POST",
      url: "/api/v1/plugins/sample-plugin/approve",
    ...
    expect(get.json().status).toBe("ENABLED");
  });

  it("403s POST .../uninstall when the Policy Engine denies CONFIGURATION.DELETE", async () => {
    await registerViaApi();
    authorizeEntityAction.mockReturnValue({
      decision: "DENIED",
      reason: "test-forced denial",
    ...
    // The plugin still exists — uninstall never ran.
    authorizeEntityAction.mockReturnValue(undefined);
    const get = await app.inject({
      method: "GET",
      url: "/api/v1/plugins/sample-plugin",
  ...
  it("still 201s a normal admin registration once the Policy Engine (real authorizeEntityAction) allows CONFIGURATION.CREATE", async () => {
    // Explicit ALLOWED decision (as opposed to the default passthrough used
    // by every other test in this file) to directly confirm the ALLOWED
    // branch, on top of the passthrough coverage already provided by the
    ...
    const res = await registerViaApi();
    expect(res.statusCode).toBe(201);
    expect(res.json().plugin.status).toBe("PENDING_REVIEW");
  });
});
