...
export function buildRegisteredAgent(id: FabricAgentId): RegisteredAgent {
  ...
  return registeredAgentSchema.parse({
    ...
    version: "1.0.0",
    status: "LAB",
    // registeredAgentSchema caps each capability at 64 chars — some catalog
    // specialty strings (e.g. LEGAL_MEDIA_COMMS) don't contain a "·"
    // separator and slugify past that limit, which used to throw a ZodError
    // out of listRegisteredAgents()/getRegisteredAgent() and crash
    // GET /api/v1/kernel/agents(/:id) entirely.
    capabilities: def.specialty
      .split("·")
      .map((s) => s.trim().toLowerCase().replace(/\s+/g, "_").slice(0, 64))
      .filter(Boolean)
      .slice(0, 8),
