...
export async function registerAdminOpsRoutes(
  ...
  app.post("/api/v1/admin/automation/run-checks", async (request) => {
    ...
    // SECOND, INDEPENDENT authorization axis (deliberate, honest 403 until a
    // real approval workflow exists): `requireAdmin` above only proves the
    // caller holds the admin *role* — it says nothing about whether this
    // particular *class* of action (triggering platform-wide automation
    // checks/remediation) has been approved. Role authority and action risk
    // are orthogonal in this design, so we additionally run the entity-
    // centric policy check. `CONFIGURATION.EXECUTE` is the highest-risk tier
    // in `DEFAULT_ENTITY_POLICIES` (`DESTRUCTIVE`, `requiresApproval: true`),
    // matching "apply/execute a control-plane automation action." With
    // `approved: false` this correctly resolves to `APPROVAL_REQUIRED`
    // rather than silently letting any admin run it — an admin is not
    // automatically entitled to trigger irreversible/high-blast-radius
    // automation without a trace.
    //
    // A future approval-workflow endpoint (e.g.
    // `POST /api/v1/admin/automation/run-checks/approve`) would need to:
    //   1. record a distinct, auditable approval decision (who approved,
    //      when, for which specific run-checks invocation/request);
    //   2. thread that approval through to this handler (e.g. a short-lived
    //      approval token or a pre-approved request id) so `approved: true`
    //      is passed here instead of a hardcoded `false`.
    // Until that exists, this endpoint intentionally 403s for every caller,
    // admin or not.
    ...
    if (entityAuthz.decision === "APPROVAL_REQUIRED") {
      throw new AtlasError(
        "APPROVAL_REQUIRED",
        "admin.automation.run-checks (CONFIGURATION.EXECUTE) is a DESTRUCTIVE-tier " +
          "control-plane action and requires an explicit approval step that does " +
          "not yet exist — admin role alone is not sufficient to run it.",
        { statusCode: 403, details: { entityType: "CONFIGURATION", action: "EXECUTE" } },
      );
    }
