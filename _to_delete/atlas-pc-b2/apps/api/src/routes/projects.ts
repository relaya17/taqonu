...
/**
 * ENTITY-LEVEL gate for the 3 mutating routes below, independent of the
 * `requireSignedInForWrite`/`assertProjectWriteAccess` identity/ownership
 * gates already applied by each one. Projects are the canonical `RECORD`
 * bucket entity (a general operational/business record) per
 * `BusinessEntityType`'s own doc comment. Same self-approved
 * signed-in-human-write pattern as `plugins.ts`/`connections.ts`/
 * `memory.ts`: `writeGateOpen`/`approved` hardcoded `true`, both
 * `DENIED`/`APPROVAL_REQUIRED` collapse to 403 `FORBIDDEN`.
 */
function enforceProjectEntityAuthz(
  action: "CREATE" | "UPDATE",
  routeLabel: string,
): void {
  const entityAuthz = authorizeEntityAction("RECORD", action, {
    mode: "WRITE",
    writeGateOpen: true,
    approved: true,
  });
  if (entityAuthz.decision !== "ALLOWED") {
    const reason =
      entityAuthz.decision === "DENIED"
        ? entityAuthz.reason
        : `${routeLabel} (RECORD.${action}) was not ALLOWED.`;
...
export async function registerProjectRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put("/api/v1/projects/:id/workspace-root", async (request) => {
    ...
    enforceProjectEntityAuthz("UPDATE", "projects.workspace-root");
    const body = z
      .object({
  ...
  app.post("/api/v1/projects", async (request, reply) => {
    ...
    enforceProjectEntityAuthz("CREATE", "projects.create");
  ...
  app.post("/api/v1/projects/:id/cloud", async (request, reply) => {
    ...
    await assertProjectWriteAccess(app, request, params.id);
    enforceProjectEntityAuthz("UPDATE", "projects.cloud-sync");
    const project = osStore.getProject(params.id);
    if (!project) {
      throw new AtlasError("NOT_FOUND", "Project not found");
    }
