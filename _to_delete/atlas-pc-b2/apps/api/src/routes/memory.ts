...
import { AtlasError, createMemorySchema, memorySchema, type AuthUser, type Memory } from "@atlas/shared";
...
export async function registerMemoryRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/memory", async (request, reply) => {
    ...
    // Entity-policy gate: creating a memory record is RECORD.CREATE
    // (LOW_RISK_WRITE, no approval required by default — mirrors how a
    // human simply saving a note doesn't need a separate approval step).
    // `writeGateOpen`/`approved` hardcoded `true` — same self-approved
